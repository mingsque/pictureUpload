...
<script>
	...
	var check = {
			...
			//중복 상활 체크 , 1.체크가 필요하다. 2.중복이다. 3.사용가능하다.
			idDuplicationFlag : "needCheck",
			...
			id : function() {
				...
				this.idDuplicationFlag = "needCheck";
				...
				if (id.length === 0) {
					...
					$("#idCheckResult").text("아이디를 4~20자리로 입력해주세요.");
				}

				else if (pattern.test(id) != true) {
					$("#idCheckResult").text("아이디는 영문과 숫자 조합입니다.");
				}
			...
			//id중복 판단 스크립트, 중복 확인 시 실행
			idDuplication : function() {
				var id = $("#id").val();
				var that = this;
				var form = {
					"id" : id
				...
				$.ajax({
					...
					async: false, //ajax 동기 비동기, 리턴값이 돌아오는걸 기다림 , 성능 생각해볼 필요가있을거같음.
					success : function(data) {
						...
						if (jObj.result === "SUCCESS") {
							...
							that.idDuplicationFlag = "checkOK";
						}
						if (jObj.result === "FALSE") {
							alert("중복된 아이디 입니다.");
							
							that.idDuplicationFlag = "duplication";
						}
					}
				});		
			},
			...
			submit : function() {
				...
				else if ( this.idDuplicationFlag === "needCheck") {
					alert("중복확인을 해주십시오");
					submitOk = false;
				} else if ( this.idDuplicationFlag === "duplication") {
					alert("중복된 아이디입니다.");
					submitOk = false;
				}
				...
				if (submitOk == true) {
					...
					$("#frm").attr("action","registerAction.do");
					$("#frm").submit();
				}		
			}			
	}
				...
				<div class="padd">
					<form class="form-horizontal" method="post" enctype="multipart/form-data" id="frm">
						...
						<div class="form-group">
							...
							<div class="col-sm-8">
								<input type="text" class="form-control" id="id" name="id" onkeyup="check.id()">
								<p style="color: red" id="idCheckResult"></p>
							</div>	
							<div class="col-sm-2">
								<button type="button" class="btn btn-default" onclick="check.idDuplication()">중복확인</button>
							</div>
						</div>
